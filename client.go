...
func loadQuiz(conn net.Conn, reader *bufio.Reader, content *fyne.Container, w fyne.Window) {
	...
	// Kreiraj labelu za prikaz poena koja će biti stalno vidljiva
	scoreLabel = widget.NewLabel(fmt.Sprintf("Tvoji poeni: %d  Protivnikovi poeni: %d", playerScore, opponentScore))
	...
	loadNextQuestion = func() {
		...
		for i := 0; i < 4; i++ {
			...
			btn.OnTapped = func() {
				...
				// Ažuriranje prikaza poena
				scoreLabel.SetText(fmt.Sprintf("Tvoji poeni: %d  Protivnikovi poeni: %d", playerScore, opponentScore))
				content.Refresh()
				...
				for j := 0; j < 4; j++ {
					...
					if j+1 == correct {
						bgColor = color.RGBA{R: 0, G: 255, B: 0, A: 255} // zeleno
					}
		...
		questionContainer := container.NewVBox(
			...
			row2,
			layout.NewSpacer(),
			scoreLabel, // Dodajemo prikaz poena
		)
